...
class MemoryEvolutionEngine:
    ...
    def strengthen_memory(self, memory_id: str) -> CareerMemoryNode:
        ...
        node.frequency += 1
        node.recency_days = 0.0  # Reset recency counter
...
if __name__ == "__main__":
    ...
    mem1 = engine.create_memory(
        user_id=user_id,
        summary="Failed Google System Design Mock Interview",
        source="INTERVIEW",
        importance=0.9,
        embedding=dummy_vector
    )
    print(f"Created Mem 1: {mem1.memory_id}, Status: {mem1.status}")
    
    # 2. Duplicate Ingestion -> Strengths memory
    ...
    print(f"Merged Mem 2 into 1: Freq={mem2.frequency}, DecayScore={mem2.decay_score}")
    
    # 3. Conflicting Ingestion -> Passed Interview
    mem3 = engine.create_memory(
        user_id=user_id,
        summary="Passed Google System Design Mock Interview",
        source="INTERVIEW",
        importance=0.95,
        embedding=[0.2] * 1024
    )
    print(f"Created Mem 3: {mem3.memory_id}, Mem 1 Status: {mem1.status}")
