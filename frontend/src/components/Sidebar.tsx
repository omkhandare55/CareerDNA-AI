...
export default function Sidebar({ isOpen = false, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed top-0 left-0 bottom-0 w-70 bg-[#0A0F1D] border-r-4 border-slate-800 flex flex-col justify-between z-50 select-none font-sans transition-transform duration-300 ease-in-out ${
      ...
      >
        ...
        <div>
          <div className="px-5 py-5 border-b-4 border-slate-800 flex items-center justify-between bg-[#020617]">
            ...
            <button
              onClick={onClose}
              className="p-1.5 text-slate-400 hover:text-white lg:hidden border border-slate-700 bg-[#0F172A]"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
